...
### 🖥️ 项目生命周期管理
- **多标签页界面**：托管项目 / 本地端口 / 全局端口 / **场景**（一等公民页：日常启停看场景，细查项目再进托管）
...
- 外部运行识别：端口被外部进程占用时标记为"外部运行中"；**cwd 归因**——监听进程的工作
  目录若对上某项目目录（或其子目录），即使项目是手动在终端启动的，也会显示
  "本项目 · 外部启动"并归到该项目名下（面板仍不会自动杀它）
...
| **默认视图 DEFAULTS** | 默认分类（all / user / creative）+ 默认标签页（managed / local / system / scenes） |
...
### v2026.09-4 — 场景升为一等公民标签页

- 🗂️ **第四个标签页「场景」** —— 日常开工/收工的主视图：场景卡片（n/m 运行中徽章 +
  启动/停止/删除）+ 步骤状态 chips（托管运行 / 外部服务 / 已停止 / 已移除，按 tone 分色）；
  点 chip 跳回「托管项目」细查。自动检测建议卡直接在页内展示（「重新检测」手动触发）
- ⚙️ `default_tab` 偏好新增 `scenes`（设置 → 默认视图），后端 `ALLOWED_TABS` 同步；
  进入场景页自动拉取列表并重跑检测
- 🛡️ 模板对场景/建议数据的 `steps` 字段加防御性兜底（stub、旧数据、竞态时序下可能缺字段，
  渲染不再抛未捕获异常）
- ✅ verify_ui 新增 P12（场景页卡片渲染 / 状态 chips 文案 / 启动请求捕获 / 建议 card /
  chip 跳转断言），--phase 12 全过、零控制台错误；基线重抓为 7 主题 × 4 标签页 28 张

### v2026.09-3 — cwd 归因：手动启动的项目也能对上端口

- 🎯 **cwd 归因** —— 端口归属不再只认"面板启动的进程树"。每条端口本来就有监听进程的
  工作目录，拿它与各项目的 cwd 对账（精确命中优先，其次"在某项目目录之下"的 monorepo
  子目录），命中即归到该项目名下（`attribution: "cwd"`）。解决边界问题：用户手动在终端
  `npm run dev`，项目卡此前只会泛泛显示"外部运行中"，现在如实显示**"本项目 · 外部启动"**
  （owner 带 `· 本项目目录`），端口条目也带上项目归属
- ⚖️ 归因只改显示不改控制权：cwd 归因的进程仍是外部进程，面板照旧不自动杀它；
  帮助弹窗给出"接管三步"指引（全局端口页终止 → 回托管页启动）
- 🐛 同目录多个项目视为歧义不归因（宁可缺 不乱归）；托管的进程树归因优先级仍最高
- ✅ 真机 E2E：手动在临时目录起 http.server → 项目卡 `external_self=true`、
  owner 含"本项目目录"、端口条目 `attribution=cwd`；pytest 82 passed（+3 用例）；
  verify_ui --phase 11 全过
